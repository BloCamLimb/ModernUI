...
public abstract class ProgramBuilder {
    ...
    public int mProjectionUniform = Engine.INVALID_RESOURCE_HANDLE;
    ...
    private boolean emitAndInstallGeomProc(String[] output) {
        ...
        assert (mProjectionUniform == Engine.INVALID_RESOURCE_HANDLE);
        ...
        for (int i = 0; i < texSamplers.length; i++) {
            ...
            texSamplers[i] = emitSampler(
                    ...
                    name);
            if (texSamplers[i] == Engine.INVALID_RESOURCE_HANDLE) {
                return false;
            }
        }
