...
public final class ResourceCache implements AutoCloseable {
    ...
    private final DequeMultiMap<Object, GpuResource> mScratchMap;
    ...
    void notifyACntReachedZero(GpuResource resource, boolean commandBufferUsage) {
        ...
        if (!resource.isCleanable() &&
                resource.getBudgetType() == Engine.BudgetType.Budgeted) {
            mFlushableCount++;
        }
        ...
        if (budgetedType == Engine.BudgetType.Budgeted) {
            ...
            if (hasUniqueKey && budgetedType == Engine.BudgetType.WrapCacheable) {
                return;
            }
    ...
    void insertResource(GpuResource resource) {
        ...
        mCount++;
        mBytes += size;
        if (resource.getBudgetType() == Engine.BudgetType.Budgeted) {
            mBudgetedCount++;
            mBudgetedBytes += size;
        }
    ...
    void didChangeBudgetStatus(GpuResource resource) {
        ...
        if (resource.getBudgetType() == Engine.BudgetType.Budgeted) {
            ...
            assert resource.getBudgetType() == Engine.BudgetType.WrapCacheable;
            mBudgetedCount--;
            mBudgetedBytes -= size;
            if (!resource.isCleanable() &&
                    !resource.hasRefOrCommandBufferUsage()) {
