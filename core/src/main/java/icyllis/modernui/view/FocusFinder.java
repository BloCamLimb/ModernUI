...
public class FocusFinder {
    ...
    private View findNextFocus(ViewGroup root, View focused, Rect focusedRect,
        ...
        return switch (direction) {
            case View.FOCUS_FORWARD, View.FOCUS_BACKWARD -> findNextFocusInRelativeDirection(
                    focusables, root, focused, direction);
            case View.FOCUS_UP, View.FOCUS_DOWN, View.FOCUS_LEFT, View.FOCUS_RIGHT -> findNextFocusInAbsoluteDirection(
                    focusables, root, focused, focusedRect, direction);
            default -> throw new IllegalArgumentException("Unknown direction: " + direction);
        };
    }

    private View findNextKeyboardNavigationCluster(View root, View currentCluster, List<View> clusters,
                                                   @View.FocusDirection int direction) {
        try {
        ...
        return switch (direction) {
            case View.FOCUS_FORWARD, View.FOCUS_DOWN, View.FOCUS_RIGHT -> getNextKeyboardNavigationCluster(root,
                    currentCluster, clusters, count);
            case View.FOCUS_BACKWARD, View.FOCUS_UP, View.FOCUS_LEFT -> getPreviousKeyboardNavigationCluster(root,
                    currentCluster, clusters, count);
            default -> throw new IllegalArgumentException("Unknown direction: " + direction);
        };
    }
    ...
    private View findNextFocusInRelativeDirection(ArrayList<View> focusables, ViewGroup root,
        ...
        return switch (direction) {
            case View.FOCUS_FORWARD -> getNextFocusable(focused, focusables, count);
            case View.FOCUS_BACKWARD -> getPreviousFocusable(focused, focusables, count);
            default -> focusables.get(count - 1);
        };
    }
    ...
    View findNextFocusInAbsoluteDirection(ArrayList<View> focusables, ViewGroup root, View focused,
        ...
        switch (direction) {
            case View.FOCUS_LEFT -> mBestCandidateRect.offset(focusedRect.width() + 1, 0);
            case View.FOCUS_RIGHT -> mBestCandidateRect.offset(-(focusedRect.width() + 1), 0);
            case View.FOCUS_UP -> mBestCandidateRect.offset(0, focusedRect.height() + 1);
            case View.FOCUS_DOWN -> mBestCandidateRect.offset(0, -(focusedRect.height() + 1));
        }
    ...
    public View findNearestTouchable(@Nonnull ViewGroup root, int x, int y, int direction, int[] deltas) {
        ...
        for (View touchable : touchables) {
            ...
            int distance = switch (direction) {
                case View.FOCUS_LEFT -> x - touchableBounds.right + 1;
                case View.FOCUS_RIGHT -> touchableBounds.left;
                case View.FOCUS_UP -> y - touchableBounds.bottom + 1;
                case View.FOCUS_DOWN -> touchableBounds.top;
            ...
            if (distance < edgeSlop) {
                ...
                if (closest == null ||
                    ...
                    switch (direction) {
                        case View.FOCUS_LEFT -> deltas[0] = -distance;
                        case View.FOCUS_RIGHT -> deltas[0] = distance;
                        case View.FOCUS_UP -> deltas[1] = -distance;
                        case View.FOCUS_DOWN -> deltas[1] = distance;
                    }
                }
            }
        }
    ...
    private static boolean isValidId(final int id) {
