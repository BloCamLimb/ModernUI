...
public class GridLayout extends ViewGroup {
    ...
    private int getDefaultMargin(View c, LayoutParams p, boolean horizontal, boolean leading) {
        ...
        boolean leading1 = (horizontal && isLayoutRtl()) != leading;
    ...
    protected boolean checkLayoutParams(ViewGroup.LayoutParams p) {
        if (!(p instanceof LayoutParams lp)) {
            return false;
        }
    ...
    @NonNull
    @Override
    protected LayoutParams generateLayoutParams(@NonNull ViewGroup.LayoutParams lp) {
    ...
    final static class Assoc<K, V> extends ArrayList<K> {
    ...
    final static class PackedMap<K, V> {
        ...
        private static <K> int[] createIndex(K[] keys) {
            ...
            for (int i = 0; i < size; i++) {
                K key = keys[i];
                Integer index = keyToIndex.computeIfAbsent(key, k -> keyToIndex.size());
                result[i] = index;
            }
            return result;
        }
    ...
    */
    static class Bounds {
        ...
        protected int getOffset(GridLayout gl, View c, Alignment a, int size, boolean horizontal) {
            //TODO 0 should be viewgroup.layout_mode
            return before - a.getAlignmentValue(c, size, 0);
        }
