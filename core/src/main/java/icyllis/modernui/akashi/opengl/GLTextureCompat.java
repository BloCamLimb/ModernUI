...
public class GLTextureCompat extends GLObjectCompat {
    ...
    public final int get() {
        if (ref == null) {
            ...
            if (target == GL11C.GL_TEXTURE_2D) {
                final int p = GL11C.glGetInteger(GL11C.GL_TEXTURE_BINDING_2D);
                GL11C.glBindTexture(GL11C.GL_TEXTURE_2D, texture);
                GL11C.glBindTexture(GL11C.GL_TEXTURE_2D, p);
            } else {
                final int p = GL11C.glGetInteger(GL32C.GL_TEXTURE_BINDING_2D_MULTISAMPLE);
                GL11C.glBindTexture(GL32C.GL_TEXTURE_2D_MULTISAMPLE, texture);
                GL11C.glBindTexture(GL32C.GL_TEXTURE_2D_MULTISAMPLE, p);
            }
    ...
    public void allocate2D(int internalFormat, int width, int height, int maxLevel) {
        ...
        GL45C.glTextureParameteri(texture, GL12C.GL_TEXTURE_BASE_LEVEL, 0);
        GL45C.glTextureParameteri(texture, GL12C.GL_TEXTURE_MAX_LEVEL, maxLevel);

        // min lod is 0 because we generate mipmap based at level 0
        // so there's no larger images
        GL45C.glTextureParameteri(texture, GL12C.GL_TEXTURE_MIN_LOD, 0);
        GL45C.glTextureParameteri(texture, GL12C.GL_TEXTURE_MAX_LOD, maxLevel);

        // the number of levels is maxLevel + 1, because it's [0, maxLevel]
        GL45C.glTextureStorage2D(texture, maxLevel + 1, internalFormat, width, height);

        long dim = 0;
        dim |= (GL45C.glGetTextureLevelParameteri(texture, 0, GL11C.GL_TEXTURE_WIDTH) & SIZE_MASK);
        dim |= (long) (GL45C.glGetTextureLevelParameteri(texture, 0, GL11C.GL_TEXTURE_HEIGHT) & SIZE_MASK) << HEIGHT_SHIFT;
        dim |= 1L << DEPTH_SHIFT;
    ...
    public void allocate2DCompat(int internalFormat, int width, int height, int maxLevel) {
        ...
        final int p = GL11C.glGetInteger(GL11C.GL_TEXTURE_BINDING_2D);
        GL11C.glBindTexture(GL11C.GL_TEXTURE_2D, get());
        GL11C.glTexParameteri(GL11C.GL_TEXTURE_2D, GL12C.GL_TEXTURE_BASE_LEVEL, 0);
        GL11C.glTexParameteri(GL11C.GL_TEXTURE_2D, GL12C.GL_TEXTURE_MAX_LEVEL, maxLevel);
        GL11C.glTexParameteri(GL11C.GL_TEXTURE_2D, GL12C.GL_TEXTURE_MIN_LOD, 0);
        GL11C.glTexParameteri(GL11C.GL_TEXTURE_2D, GL12C.GL_TEXTURE_MAX_LOD, maxLevel);

        for (int level = 0; level <= maxLevel; level++) {
            GL11C.nglTexImage2D(GL11C.GL_TEXTURE_2D, level, internalFormat, width >> level,
                    height >> level, 0, GL11C.GL_RED, GL11C.GL_UNSIGNED_BYTE, MemoryUtil.NULL);
        }

        long dim = 0;
        dim |= (GL11C.glGetTexLevelParameteri(GL11C.GL_TEXTURE_2D, 0, GL11C.GL_TEXTURE_WIDTH) & SIZE_MASK);
        dim |= (long) (GL11C.glGetTexLevelParameteri(GL11C.GL_TEXTURE_2D, 0, GL11C.GL_TEXTURE_HEIGHT) & SIZE_MASK) << HEIGHT_SHIFT;
        dim |= 1L << DEPTH_SHIFT;
        if (dimension != 0 && dimension != dim) {
    ...
    public void allocate2DMS(int internalFormat, int width, int height, int samples) {
        if (target != GL32C.GL_TEXTURE_2D_MULTISAMPLE) {
            throw new IllegalStateException();
        }
        if (samples < 1) {
            throw new IllegalArgumentException();
        ...
        final int texture = get();
        GL45C.glTextureStorage2DMultisample(texture, samples, internalFormat, width, height, true);
    ...
    public void upload(int level, int x, int y, int width, int height, int rowLength, int skipRows,
        ...
        GL45C.nglTextureSubImage2D(get(), level, x, y, width, height, format, type, pixels);
    }
    ...
    public void uploadCompat(int level, int x, int y, int width, int height, int rowLength, int skipRows,
        ...
        GL11C.glPixelStorei(GL11C.GL_UNPACK_ALIGNMENT, alignment);
        final int p = GL11C.glGetInteger(GL11C.GL_TEXTURE_BINDING_2D);
        GL11C.glBindTexture(target, get());
        GL11C.nglTexSubImage2D(target, level, x, y, width, height, format, type, pixels);
        GL11C.glBindTexture(target, p);
    }
    ...
    public void clear(int level) {
        GL44C.nglClearTexImage(get(), level, GL11C.GL_RED, GL11C.GL_UNSIGNED_BYTE, MemoryUtil.NULL);
    }

    public void clear(int level, int x, int y, int width, int height) {
        GL44C.nglClearTexSubImage(get(), level, x, y, 0, width, height, 1, GL11C.GL_RED, GL11C.GL_UNSIGNED_BYTE, MemoryUtil.NULL);
    }
    ...
    public void setWrapS(int wrapS) {
        GL45C.glTextureParameteri(get(), GL11C.GL_TEXTURE_WRAP_S, wrapS);
    }
    ...
    public void setWrapT(int wrapT) {
        GL45C.glTextureParameteri(get(), GL11C.GL_TEXTURE_WRAP_T, wrapT);
    }
    ...
    public void setWrapR(int wrapR) {
        GL45C.glTextureParameteri(get(), GL12C.GL_TEXTURE_WRAP_R, wrapR);
    }
    ...
    public void setWrap(int wrapS, int wrapT) {
        final int texture = get();
        GL45C.glTextureParameteri(texture, GL11C.GL_TEXTURE_WRAP_S, wrapS);
        GL45C.glTextureParameteri(texture, GL11C.GL_TEXTURE_WRAP_T, wrapT);
    }
    ...
    public void setWrapMode(int wrapS, int wrapT, int wrapR) {
        ...
        GL45C.glTextureParameteri(texture, GL11C.GL_TEXTURE_WRAP_S, wrapS);
        GL45C.glTextureParameteri(texture, GL11C.GL_TEXTURE_WRAP_T, wrapT);
        GL45C.glTextureParameteri(texture, GL12C.GL_TEXTURE_WRAP_R, wrapR);
    }
    ...
    public void setFilter(boolean linear, boolean mipmap) {
        ...
        if (linear) {
            if (mipmap) {
                GL45C.glTextureParameteri(texture, GL11C.GL_TEXTURE_MIN_FILTER, GL11C.GL_LINEAR_MIPMAP_LINEAR);
            } else {
                GL45C.glTextureParameteri(texture, GL11C.GL_TEXTURE_MIN_FILTER, GL11C.GL_LINEAR);
            }
            GL45C.glTextureParameteri(texture, GL11C.GL_TEXTURE_MAG_FILTER, GL11C.GL_LINEAR);
        } else {
            if (mipmap) {
                GL45C.glTextureParameteri(texture, GL11C.GL_TEXTURE_MIN_FILTER, GL11C.GL_NEAREST_MIPMAP_LINEAR);
            } else {
    ...
    public void setFilter(int minFilter, int magFilter) {
        final int texture = get();
        GL45C.glTextureParameteri(texture, GL11C.GL_TEXTURE_MIN_FILTER, minFilter);
        GL45C.glTextureParameteri(texture, GL11C.GL_TEXTURE_MAG_FILTER, magFilter);
    }
    ...
    public void setSwizzle(int r, int g, int b, int a) {
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            GL45C.glTextureParameteriv(get(), GL33C.GL_TEXTURE_SWIZZLE_RGBA, buffer);
        }
    }
    ...
    public void setSwizzleCompat(int r, int g, int b, int a) {
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            final int p = GL11C.glGetInteger(GL11C.GL_TEXTURE_BINDING_2D);
            GL11C.glBindTexture(target, get());
            GL11C.glTexParameteriv(target, GL33C.GL_TEXTURE_SWIZZLE_RGBA, buffer);
            GL11C.glBindTexture(target, p);
        }
    }
    ...
    public void generateMipmap() {
        GL45C.glGenerateTextureMipmap(get());
    }
    ...
    public int getInternalFormat() {
        return GL45C.glGetTextureLevelParameteri(get(), 0, GL11C.GL_TEXTURE_INTERNAL_FORMAT);
    }
    ...
    private static final class Ref extends GLObjectCompat.Ref {
        ...
        @Override
        public void run() {
            Core.postOnRenderThread(() -> GL11C.glDeleteTextures(mId));
        }
    }
}
