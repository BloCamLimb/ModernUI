...
public class GLRenderTarget extends RenderTarget {
    ...
    public static GLRenderTarget makeWrapped(GLServer server,
        ...
        if (stencilBits > 0) {
            ...
            int stencilFormat = switch (stencilBits) {
                ...
                case 8 -> GL30C.GL_DEPTH24_STENCIL8;
                case 16 -> GL30C.GL_STENCIL_INDEX16;
                default -> 0;
            };
    ...
    public void bindStencil() {
        ...
        if (stencilBuffer != null) {
            GL45C.glNamedFramebufferRenderbuffer(framebuffer,
    ...
    protected void deallocate() {
        ...
        if (mOwnership) {
            if (mRenderFramebuffer != 0) {
                GL30C.glDeleteFramebuffers(mRenderFramebuffer);
            }
            if (mRenderFramebuffer != mResolveFramebuffer) {
                assert (mResolveFramebuffer != 0);
                GL30C.glDeleteFramebuffers(mResolveFramebuffer);
            }
        }
