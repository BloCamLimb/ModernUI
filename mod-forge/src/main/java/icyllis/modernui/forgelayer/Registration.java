...
import icyllis.modernui.forgelayer.network.NetworkHandler;
import icyllis.modernui.mcimpl.MuiModRegistries;
...
public final class Registration {
    ...
    static void registerSounds(@Nonnull RegistryEvent.Register<SoundEvent> event) {
        ...
        MuiModRegistries.BUTTON_CLICK_1 = registerSound(registry, "button1");
        MuiModRegistries.BUTTON_CLICK_2 = registerSound(registry, "button2");
    }
    ...
    static void registerMenus(@Nonnull RegistryEvent.Register<MenuType<?>> event) {
        ...
        MuiModRegistries.TEST_MENU = registerMenu(registry, TestMenu::new, "test");
    }

    @SubscribeEvent
    static void registerItems(@Nonnull RegistryEvent.Register<Item> event) {
        if (ModernUI_Forge.alphaTest) {
            Item.Properties properties = new Item.Properties().stacksTo(1).setISTER(() -> ProjectBuilderRenderer::new);
            event.getRegistry().register(MuiModRegistries.PROJECT_BUILDER_ITEM = new Item(properties).setRegistryName("project_builder"));
        }
    }
    ...
    static void setupClient(@Nonnull FMLClientSetupEvent event) {
        ...
        ModernUI_Forge.EVENT_BUS.register(EventHandler.Internal.class);
