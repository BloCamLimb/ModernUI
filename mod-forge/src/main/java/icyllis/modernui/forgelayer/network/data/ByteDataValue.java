...
public class ByteDataValue extends DataValue {

    private byte value;

    ByteDataValue(Runnable dirty, @Nullable String key) {
        super(dirty, key);
    }
    ...
    public void write(@Nonnull CompoundNBT compoundNBT) {
        ...
        compoundNBT.putByte(key, value);
    }

    @Override
    public void read(@Nonnull CompoundNBT compoundNBT) {
        Objects.requireNonNull(key, "Accessing NBT value but key is unspecified");
        value = compoundNBT.getByte(key);
    }
