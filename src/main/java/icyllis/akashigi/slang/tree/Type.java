...
public class Type extends Symbol {
    ...
    public final boolean canCoerceTo(Type other) {
        return CoercionCost.isPossible(getCoercionCost(other));
    }
    ...
    public final long getCoercionCost(Type other) {
        ...
        if (isNumeric() && other.isNumeric()) {
            if (getScalarKind() != other.getScalarKind()) {
                ...
                return CoercionCost.narrow(getRank() - other.getRank());
            }
        }
        if (isGeneric()) {
    ...
    public final Expression coerceExpression(Expression expr) {
        ...
        if (!CoercionCost.isPossible(expr.getCoercionCost(this))) {
    ...
    public static final class CoercionCost {
        ...
        private static final long IMPOSSIBLE = 0x80000000_80000000L;
        ...
        public static long narrow(int cost) {
            assert cost >= 0;
            return (long) cost << 32;
        }
        ...
        public static boolean isPossible(long cost) {
            return (cost & IMPOSSIBLE) == 0;
        }
        ...
        public static long concat(long lhs, long rhs) {
            ...
            if (((lhs & IMPOSSIBLE) | (rhs & IMPOSSIBLE)) != 0) {
                return IMPOSSIBLE;
            }
            // overflow becomes impossible (should not happen)
            long normal = (int) lhs + (int) rhs;
            long narrow = (lhs >> 32) + (rhs >> 32);
            return normal | (narrow << 32);
        }

        public static int compare(long lhs, long rhs) {
            // any negative value means impossible
            int res = Boolean.compare((lhs & IMPOSSIBLE) != 0, (rhs & IMPOSSIBLE) != 0);
            if (res != 0) {
                return res;
            }
