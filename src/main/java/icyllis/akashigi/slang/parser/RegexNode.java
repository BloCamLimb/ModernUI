...
public interface RegexNode {
     ...
     */
    @Nonnull
    static RegexNode CharClass(RegexNode[] clazz, boolean negative) {
    ...
    class CharClass extends BitSet implements RegexNode {
        ...
        private CharClass(RegexNode[] clazz, boolean negative) {
            mNegative = negative;
        ...
        @Override
        public IntList transition(NFA nfa, @Unmodifiable IntList next) {
            int state = nfa.add(NFAState.Filter(ch -> get(ch) ^ mNegative, next));
            return IntList.of(state);
        }

        @Override
        public String toString() {
            return "CharClass(" + (mNegative ? "^" : "") + super.toString() + ")";
        }
    }
}
