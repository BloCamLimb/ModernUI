 ...
 * {@link ResourceCache}.
 * <p>
 * Each {@link GpuResourceBase} should be created with immutable GPU memory allocation.
 * Use {@link ResourceProvider} to obtain {@link GpuResourceBase} objects.
 */
abstract sealed class GpuResourceBase implements GpuResource permits GpuImageBase, GpuFramebuffer, GpuBuffer {
    ...
    static {
        ...
        try {
            REF_CNT = lookup.findVarHandle(GpuResourceBase.class, "mRefCnt", int.class);
            COMMAND_BUFFER_USAGE_CNT = lookup.findVarHandle(GpuResourceBase.class, "mCommandBufferUsageCnt", int.class);
    ...
    static final PriorityQueue.Access<GpuResourceBase> QUEUE_ACCESS = new PriorityQueue.Access<>() {
        @Override
        public void setIndex(GpuResourceBase resource, int index) {
            resource.mCacheIndex = index;
        }

        @Override
        public int getIndex(GpuResourceBase resource) {
    ...
    protected GpuResourceBase(GpuDevice device) {
        assert (device != null);
        mDevice = device;
    }
