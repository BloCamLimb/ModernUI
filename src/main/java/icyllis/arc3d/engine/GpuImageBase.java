...
/**
 * Represents 2D/3D images/textures/attachments. A GpuImage
 * may or may not be sampled by shaders, may be used as color/depth/stencil attachments
 * of render targets. See {@link ISurface#FLAG_TEXTURABLE} and {@link ISurface#FLAG_RENDERABLE}.
 * <p>
 ...
 * are reused together, see {@link GpuFramebuffer}.
 */
public abstract non-sealed class GpuImageBase extends GpuResourceBase implements GpuImage {
    ...
    protected GpuImageBase(GpuDevice device, int width, int height) {
        super(device);
     ...
     *  set, otherwise not budgeted. {@link GpuImageBase} only.
     * </li>
     *
     * <li>{@link ISurface#FLAG_MIPMAPPED} -
     *  Used to say whether a texture has mip levels allocated or not. Mipmaps are allocated
     *  when set, otherwise mipmaps are not allocated. {@link GpuImageBase} only.
     * </li>
     *
     ...
     *  Means the pixels in the texture are read-only. {@link GpuImageBase} only.
     * </li>
     *
